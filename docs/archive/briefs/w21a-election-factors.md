...
Read `docs/FRAMEWORK.md`. Mainline at `<mainline-checkout>` is READ-ONLY. You own `packages/engine/src/electionEngine/` (new directory) plus test files and the single export line in `index.ts`. Do NOT touch world.ts, save.ts, types.ts, phases, or any existing module: this wave is strictly the pure formula layer; the resolution pipeline that composes these is a separate operator-led wave.
