...
1. **Launcher screen** as the app entry: three clear entries (Play Online, New World, Load Save), monochrome design language consistent with `src/app.css` (dark, sharp, no serif, no beige). Keep it plain and fast, not splashy.
2. **Online mode**: opens `https://www.ahousedividedgame.com` in a **dedicated Tauri window** with zero capabilities: no fs, no dialog, no IPC exposure to remote content. The main window stays open (or offers a way back). Verify against FRAMEWORK.md security doctrine; if a Tauri v2 default would leak IPC or capabilities to the remote window, lock it down explicitly in `tauri.conf.json` / capabilities files.
