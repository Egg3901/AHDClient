...
Read `docs/FRAMEWORK.md`. Mainline at `<mainline-checkout>` is READ-ONLY. You own `packages/engine/src/electionEngine/tally/` (new subdirectory) plus tests and export lines in the electionEngine index. Do NOT touch anything else: no world.ts, save.ts, types.ts, phases (the operator is building orchestration in `src/elections/` in parallel; stay out of that directory too).
