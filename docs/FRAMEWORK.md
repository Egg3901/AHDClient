...
3a. **The Ask panel is local UI with a Rust proxy.** The `ask` window shows `index.html?view=ask` and may invoke only `ask_api`, `ask_send`, `ask_stop` (`capabilities/ask.json`); those commands carry the Ask session cookie server-side of the webview boundary and reach an exact route allowlist. The one-time sign-in window (`ask-auth`) loads the Ask origin under the Ask navigation guard with zero capabilities and is closed by the watcher the moment the session lands. Never enable remote-domain IPC access.
